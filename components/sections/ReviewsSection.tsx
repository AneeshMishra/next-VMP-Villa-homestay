    ...
    <div style={{ padding: "80px 40px" }} id="reviews">
      <div className="max-w-[1100px] mx-auto">
        <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
          <div>
            <div className="text-[11px] font-bold tracking-[2.5px] uppercase text-saffron mb-2.5">Guest Love</div>
            <h2 className="font-display font-bold text-ink mb-2 leading-tight" style={{ fontSize: "clamp(28px, 4vw, 44px)" }}>What travellers <em className="not-italic text-saffron">say about us</em></h2>
            <p className="text-muted text-[15px]">Collected from Booking.com, TripAdvisor, Google, and MakeMyTrip — unfiltered and real.</p>
          </div>
              ...
              <ScrollReveal key={r.name + i} delay={i * 80}>
                <div className="bg-white rounded-2xl p-6 border border-marble transition-shadow duration-200 hover:shadow-lg">
                  <div className="flex items-center gap-3 mb-3.5">
                    <div className="w-[42px] h-[42px] rounded-full flex items-center justify-center font-bold text-base text-white shrink-0" style={{ background: r.avatarColor }}>{r.initial}</div>
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm text-ink">{r.name}</div>
                      <div className="text-[11px] text-stone">{r.meta}</div>
                    </div>
                    <div className="text-xs text-saffron tracking-wide ml-auto">{"★".repeat(r.stars)}{r.stars < 5 && <span className="text-marble">{"★".repeat(5 - r.stars)}</span>}</div>
                  </div>
                  <p className="text-[13px] leading-[1.7] text-muted italic">{r.text}</p>
                  <span className="inline-block mt-3.5 text-[10px] font-bold tracking-wide uppercase py-1 px-2 rounded" style={style}>{r.source}</span>
                </div>
              </ScrollReveal>
            );
          })}
