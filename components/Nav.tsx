    ...
    <>
      ...
      >
        ...
        <div className="hidden md:flex gap-8 ml-auto mr-8">
          {NAV_LINKS.map((l) => (
            <Link key={l.href} href={l.href} className="text-white/70 text-sm font-medium tracking-wide hover:text-white transition-colors">{l.label}</Link>
          ))}
        </div>
      ...
      <div className={`fixed inset-0 bg-ink z-[99] flex flex-col items-center justify-center gap-8 transition-all duration-300 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        ...
        {NAV_LINKS.map((l) => (
          <Link key={l.href} href={l.href} className="text-white/85 text-2xl font-medium hover:text-saffron transition-colors" onClick={() => setMenuOpen(false)}>{l.label}</Link>
        ))}
