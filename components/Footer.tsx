      ...
      <div className="max-w-[1100px] mx-auto">
        <div className="grid gap-12 pb-10 mb-10" style={{ gridTemplateColumns: "1.5fr 1fr 1fr 1fr", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          <div>
            ...
            <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1.5 rounded-full" style={{ background: "rgba(58,107,74,0.2)", border: "1px solid rgba(58,107,74,0.3)", color: "#6FCF97" }}>🌿 Eco-Certified Homestay</span>
          </div>
          <div>
            <div className="text-[11px] font-bold tracking-widest text-white/80 mb-4 uppercase">Quick Links</div>
            <div className="flex flex-col gap-2.5">
              {[{href:"/rooms",label:"Our Rooms"},{href:"/facilities",label:"Facilities"},{href:"/location",label:"Location & Map"},{href:"/contact#reviews",label:"Guest Reviews"},{href:"/faq",label:"FAQ"}].map((l) => (
                <Link key={l.href} href={l.href} className="text-sm text-white/45 hover:text-white transition-colors">{l.label}</Link>
              ))}
            </div>
          </div>
          <div>
            <div className="text-[11px] font-bold tracking-widest text-white/80 mb-4 uppercase">Room Types</div>
            <div className="flex flex-col gap-2.5">
              {[{href:"/rooms#deluxe-ac",label:"Deluxe AC Room"},{href:"/rooms#standard-ac",label:"Standard AC Room"},{href:"/rooms#dormitory",label:"Dormitory Bed"},{href:"/#booking",label:"Check Availability"},{href:"/contact",label:"Special Requests"}].map((l) => (
                <Link key={l.href} href={l.href} className="text-sm text-white/45 hover:text-white transition-colors">{l.label}</Link>
              ))}
            </div>
          </div>
          <div>
            <div className="text-[11px] font-bold tracking-widest text-white/80 mb-4 uppercase">Contact</div>
            <div className="flex flex-col gap-2.5 text-sm">
              <div className="flex gap-2.5"><span className="opacity-60">📍</span><span>{ADDRESS}</span></div>
              <div className="flex gap-2.5"><span className="opacity-60">📞</span><a href={`tel:+${PHONE_DISPLAY.replace(/\D/g,"")}`} className="text-white/45 hover:text-white transition-colors">{PHONE_DISPLAY}</a></div>
